...
func GenerateReportMarkdown(result ChecksResult) string {
	...
	// Deliberately no timestamp: the report is committed back to the consumer's
	// repository, so a generated-at line would produce a one-line diff on every
	// run and defeat the "unchanged, skip the commit" guard in commit-badge.sh.
